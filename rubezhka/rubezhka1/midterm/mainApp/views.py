...
@csrf_exempt
def blog_list_handler(request):
    if request.method == 'GET':
        categories = Blog.objecsts.all()
        serializer = BlogsSerializers(categories, many=True)
        return JsonResponse(serializer.data, status = 200, safe = False)
    if request.method == 'POST':
        data = json.loads(request.body)
        seralizer = BlogsSerializers(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(data = serializer.data, status = 200)
        return JsonResponse(data=serializer.erros, status = 400)
...
def category_products_handler(request, pk):
    ...
    if request.method == 'GET':
        ...
        serializer = BlogsSerializers(products, many=True)
        return JsonResponse(serializer.data, safe=False)
    if request.method == 'POST':
        data = json.loads(request.body)
        data['category_id'] = pk
        serializer = BlogsSerializers(data=data)
        if serializer.is_valid():
            serializer.save()
